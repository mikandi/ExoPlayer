...
public class DashRendererBuilder implements RendererBuilder, ManifestCallback<MediaPresentationDescription>
{
	...
	public void onManifest(String contentId, MediaPresentationDescription manifest)
	{
		...
		if (hasContentProtection)
		{
			...
			try
			{
				...
				filterHdContent =
						videoAdaptationSet != null && videoAdaptationSet.hasContentProtection() && !drmSessionManagerData.second;
			}
		...
		if (videoAdaptationSet != null)
		{
			...
			for (int i = 0; i < videoRepresentations.size(); i++)
			{
				...
				else if (!format.mimeType.equals(MimeTypes.VIDEO_MP4) && !format.mimeType.equals(MimeTypes.VIDEO_WEBM))
				{
		...
		else
		{
			...
			ChunkSource videoChunkSource =
					new DashChunkSource(manifestFetcher, videoAdaptationSetIndex, videoRepresentationIndices, videoDataSource,
							new AdaptiveEvaluator(bandwidthMeter), LIVE_EDGE_LATENCY_MS);
			ChunkSampleSource videoSampleSource =
					new ChunkSampleSource(videoChunkSource, loadControl, VIDEO_BUFFER_SEGMENTS * BUFFER_SEGMENT_SIZE, true,
							mainHandler, player, DemoPlayer.TYPE_VIDEO);
			videoRenderer =
					new MediaCodecVideoTrackRenderer(videoSampleSource, drmSessionManager, true,
							MediaCodec.VIDEO_SCALING_MODE_SCALE_TO_FIT, 5000, null, mainHandler, player, 50);
			debugRenderer =
					debugTextView != null ? new DebugTrackRenderer(debugTextView, videoRenderer, videoSampleSource) : null;
		}
		...
		if (audioAdaptationSet != null)
		{
			...
			for (int i = 0; i < audioRepresentations.size(); i++)
			{
				...
				audioChunkSourceList.add(new DashChunkSource(manifestFetcher, audioAdaptationSetIndex, new int[] { i },
		...
		else
		{
			...
			SampleSource audioSampleSource =
					new ChunkSampleSource(audioChunkSource, loadControl, AUDIO_BUFFER_SEGMENTS * BUFFER_SEGMENT_SIZE, true,
							mainHandler, player, DemoPlayer.TYPE_AUDIO);
			// TODO: There needs to be some logic to filter out non-AC3 tracks when selecting to use AC3.
			boolean useAc3Passthrough =
					haveAc3Tracks && audioCapabilities != null
							&& (audioCapabilities.supportsAc3() || audioCapabilities.supportsEAc3());
			if (useAc3Passthrough)
			{
				audioRenderer = new Ac3PassthroughAudioTrackRenderer(audioSampleSource, mainHandler, player);
			}
			else
			{
		...
		else
		{
			...
			SampleSource textSampleSource =
					new ChunkSampleSource(textChunkSource, loadControl, TEXT_BUFFER_SEGMENTS * BUFFER_SEGMENT_SIZE, true,
							mainHandler, player, DemoPlayer.TYPE_TEXT);
			textRenderer =
					new TextTrackRenderer(textSampleSource, player, mainHandler.getLooper(), new TtmlParser(), new WebvttParser());
		}
	...
	private static class V18Compat
	{
		...
		public static Pair<DrmSessionManager, Boolean> getDrmSessionManagerData(DemoPlayer player, MediaDrmCallback drmCallback)
				throws UnsupportedDrmException
		{
			try
			{
				StreamingDrmSessionManager streamingDrmSessionManager =
						new StreamingDrmSessionManager(DemoUtil.WIDEVINE_UUID, player.getPlaybackLooper(), drmCallback, null,
		...
		private static int getWidevineSecurityLevel(StreamingDrmSessionManager sessionManager)
		{
			...
			return securityLevelProperty.equals("L1") ? SECURITY_LEVEL_1 : securityLevelProperty.equals("L3") ? SECURITY_LEVEL_3
